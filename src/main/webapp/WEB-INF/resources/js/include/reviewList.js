...
const current_id = document.getElementById('current_id').value;
...
function handler2(json){
	...
	for(const rec of reviewList){
		...
		if(`${ current_id == rec.memberVO.member_id }` == 'true'){
		div.innerHTML += `<div class="review__button__cont"><button class="modify">수정</button> | <button class="remove">삭제</button></div>`;
		}
		...
		p1.textContent = rec.memberVO.nickname
