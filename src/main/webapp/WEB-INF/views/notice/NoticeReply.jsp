...
<!-- 자바스크립트 -->
<script defer src="${contextPath }/js/notice/NoticeReply.js"></script>
<script defer src="https://kit.fontawesome.com/946dc76c2e.js" crossorigin="anonymous"></script>
<!-- ck에디터 -->
...
<body>
	...
	<section  id="card_slider">
		    	<p id="title">공지사항/이벤트 수정</p>
    	<hr>
    		<!-- 글쓰기 폼 시작 -->
    		<form method="post" 
    					id="NoticeWriteForm"
    					action="${contextPath }/notice/modify"
    		>
  			
					
			    	<label for="ntitle">제목</label>
			    	<input type=text id="ntitle" name="ntitle" value="[원글]${NoticeVO.ntitle }"/>
    			<span>
					...
					<textarea name="ncontent" id="ncontent" style="display:none">
						[원글]${NoticeVO.ncontent }
					</textarea>
	  		...
	  		<input type="hidden" name="nid" 			value="${sessionScope.member.member_id }"/>
			<input type="hidden" name="nnickname" 		value="${sessionScope.member.nickname }"/>
			<input name="nnum" id="nnum" type="hidden" value="${NoticeVO.nnum }" readonly="readonly" />			
    		</form>
    		<!-- 글쓰기 폼 끝 -->
		  <div class="text-end">
  		<!-- 취소 버튼(작성자만) -->
		  <c:if test="${selectNid eq sessionScope.member.member_id }">
				<button id="cancleBtn"
						type="button" 
				        class="btn rmode btn-outline-dark btn-sm">취소</button>
		  </c:if>
		  	<!-- 공통버튼 -->
